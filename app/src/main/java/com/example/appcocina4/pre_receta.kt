...
class pre_receta : AppCompatActivity() {
    ...
    fun obtenerListaPasos(){
        db.collection("recetas").document("Lasaña").collection("Info").document("Instrucciones").get().addOnSuccessListener {
                lasana ->
            var tempnum = 1
            /*
            ...
            */
            var Pasos = lasana.toObject(Instrucciones::class.java)
    ...
    fun obtenerNumeroPasos(){
        db.collection("recetas").document("Lasaña").get().addOnSuccessListener { tempLasana ->

            val num  = tempLasana.data?.get("numeroPasos").toString()
            pasos_totales = num.toInt()

        }.addOnFailureListener{
                _ -> println("error aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa en numero")
        }
    }
